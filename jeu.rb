class Personne
  ...
  def attaque(personne)
    ...
    # - Affiche ce qu'il s'est passé
      ...
      if self.class == Joueur
        puts "#{@nom} posséde #{degats_bonus} de bonus d'attaque."
      end
  ...
  def subit_attaque(degats_recus)
    ...
    puts "#{@nom} subit #{degats_recus} point de dégats"
...
class Joueur < Personne
  ...
  def soin
    ...
    # - Affiche ce qu'il s'est passé
      if @points_de_vie >= 100
        puts "#{@nom} ne peut se soigner n'étant pas blessé."
      else
        s = 1 + Random.rand(40)
        ...
        puts "#{@nom} a regagne #{s} points de vie (#{@points_de_vie} /100)."
      end
  end
  ...
  def ameliorer_degats
    ...
    b = 5 + Random.rand(30)
    @degats_bonus += b
    puts "#{@nom} gagne #{b} points de bonus de dégat."
  end
end
